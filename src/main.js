...
const ui = getUI();
// --- task mode buttons (P/PD/PID) ---
const taskModeHidden = document.getElementById("taskMode");      // hidden input
const taskModeBtns = document.getElementById("taskModeBtns");    // button container

function setTaskMode(mode){
  taskModeHidden.value = mode;

  // active表示更新
  taskModeBtns.querySelectorAll(".modeBtn").forEach(b=>{
    b.classList.toggle("is-active", b.dataset.mode === mode);
  });

  // 既存の ui.taskMode.onchange を動かす（PID reset等）
  taskModeHidden.dispatchEvent(new Event("change"));
}

taskModeBtns.addEventListener("click", (e)=>{
  const btn = e.target.closest(".modeBtn");
  if (!btn) return;
  setTaskMode(btn.dataset.mode);
});

// 初期状態を反映
setTaskMode(taskModeHidden.value || "PID");
